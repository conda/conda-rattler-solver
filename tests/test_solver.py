...
def test_installed_update_all_benchmark(
...
@pytest.mark.xfail(
    reason="known issue: https://github.com/conda/conda-rattler-solver/issues/88",
    strict=True,
)
@pytest.mark.parametrize(
    "channel_priority",
...
def test_channel_priority_updates_installed_dependency(
    ...
    """
    An active channel that publish a newer `foo` must take precedence
...
def test_channel_priority_updates_installed_dependency_two(
    ...
    """
    An active channel that to publish a newer `foo` must take precedence
...
def test_can_update_env_with_python(
    ...
    """
    Real world example, ensure that we can run an update when python is in the environment
    """
